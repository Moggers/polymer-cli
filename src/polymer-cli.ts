...
import {ProjectConfig, ProjectConfigOptions} from 'polymer-project-config';
 ...
 * "lowerCamelCase". This helper function converts the special commandLineArgs
 * formatted data with its hyphen-case flags to a baseic options object with
 * lowerCamelCase properties.
 */
function parseCLIArgs(commandOptions: any): {[name: string]: string} {
  ...
  // if any mappings are required from "hyphen-case" -> "lowerCamelCase",
  // do them here.
...
export class PolymerCli {
  ...
  args: string[];
  defaultConfigOptions: ProjectConfigOptions;

  constructor(args: string[], configOptions?: ProjectConfigOptions) {
